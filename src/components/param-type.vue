<template>
  <div class="justify-content-sm-start">
    ...
    <span v-if="!hasArray && !hasPromise">
      <router-link v-if="hasLink.length"
                   :to="{name: `documentation`,params: { name: hasLink[0], member: hasLink[1] || undefined },}">
        {{ name }}
      </router-link>
      <span v-else>{{ name }}</span>
    </span>
    <param-type v-if="hasArray || hasPromise"
                :type="name"
...
export default class ParamType extends Vue {
  ...
  @Watch(`type`, { immediate: true,}) watchType(type: string) {
    if (type?.search(/^Array\.</) === 0) this.hasArray = true;
    if (type?.search(/^Promise\.</) === 0) this.hasPromise = true;
